...
def data_generation(num_x, num_y, num_t, NumOfSamples, nx_s, ny_s, freq, river, low, high, cov_l):
    ...
    for j in range(NumOfSamples):
        ...
        vv=Concat(vv, TakeAverage(dd, num_y, num_x, ny_s, nx_s))
        river_v=Concat(river_v, TakeAverage(river_d, num_y, num_x, ny_s, nx_s))
        temp_v=Concat(temp_v, TakeAverage(landscape+dd+0.1*noise, num_y, num_x, ny_s, nx_s))
